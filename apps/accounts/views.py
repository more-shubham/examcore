...
class AuthView(View):
    ...
    def get_mode(self, request):
        ...
        if registration.get('email') and registration.get('password') and not registration.get('otp_verified'):
            return 'verify'
    ...
    def handle_school(self, request):
        ...
        if form.is_valid():
            ...
            messages.success(request, f"Welcome to {school.name}! Your account is ready.")
            return redirect('accounts:dashboard')
...
class DashboardView(LoginRequiredMixin, View):
    ...
    def get(self, request):
        ...
        # Examiner dashboard (coming soon - for now same as student)
        if user.is_examiner:
            return render(request, 'accounts/dashboard_examiner.html', {
                'school': school,
                'user': user,
            })
        ...
        context = {
            'school': school,
            'user': user,
            'examiner_count': User.objects.filter(role=User.Role.EXAMINER, is_active=True).count(),
            ...
            'student_count': User.objects.filter(role=User.Role.STUDENT, is_active=True).count(),
        }
...
class ExaminerManagementView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        context = {
            'users': User.objects.filter(role=User.Role.EXAMINER).order_by('first_name', 'last_name'),
...
class TeacherManagementView(AdminRequiredMixin, View):
    ...
    def get(self, request):
        context = {
            'users': User.objects.filter(role=User.Role.TEACHER).order_by('first_name', 'last_name'),
            'form': AddTeacherForm(),
        }
        return render(request, self.template_name, context)
...
class StudentManagementView(AdminRequiredMixin, View):
    ...
    def get(self, request, class_id):
        ...
        context = {
            ...
            'users': User.objects.filter(role=User.Role.STUDENT, assigned_class=selected_class).order_by('first_name', 'last_name'),
